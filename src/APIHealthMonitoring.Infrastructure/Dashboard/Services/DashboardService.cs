...
public class DashboardService : IDashboardService
{
    ...
    {
        // Cache-aside
        if (_cache.TryGetValue<DashboardSummaryDto>(CacheKeys.DashboardSummary, out var cached) && cached is not null)
            return cached;
        ...
        _cache.Set(CacheKeys.DashboardSummary,
            result,
            TimeSpan.FromSeconds(_cacheSettings.DashboardSummaryExpirationSeconds));
    ...
    {
        var cacheKey = $"{CacheKeys.DashboardApiCards}:{pageIndex}:{pageSize}";

        if (_cache.TryGetValue<PaginatedResult<ApiDashboardCardDto>>(cacheKey, out var cached) && cached is not null)
            return cached;
        ...
        var result = new PaginatedResult<ApiDashboardCardDto>(cards, totalItems, pageIndex, pageSize);

        _cache.Set(cacheKey,
            result,
            TimeSpan.FromSeconds(_cacheSettings.DashboardApiCardsExpirationSeconds));
    ...
    {
        var cacheKey = $"{CacheKeys.ApiStatsPrefix}{apiEndpointId}";

        if (_cache.TryGetValue<ApiHistoricalStatsDto>(cacheKey, out var cached) && cached is not null)
            return cached;
        ...
        var result = new ApiHistoricalStatsDto
        {
            ApiEndpointId       = endpoint.Id,
        ...
        _cache.Set(cacheKey,
            result,
            TimeSpan.FromSeconds(_cacheSettings.ApiStatsExpirationSeconds));
